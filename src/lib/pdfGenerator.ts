...
export const generateBillPDF = async (billData: BillData): Promise<void> => {
  ...
  const phoneNumber = billData.customer.phone.length > 12
    ? `${billData.customer.phone.substring(0, 12)}...`
  ...
  pdf.text('Gold Value', 205, yPos + 7, { align: 'center' })
  pdf.text('Making Charges', 230, yPos + 7, { align: 'center' })
  pdf.text('Hallmark', 257.5, yPos + 7, { align: 'center' })
  ...
  billData.items.forEach((item, index) => {
    ...
    const makingChargeAmount = item.makingCharges ?? 0
    const goldPortion = Math.max(item.amount - makingChargeAmount, 0)
    const weight = item.weight ?? 0
    const goldRatePerGram = weight ? goldPortion / weight : 0
    ...
    pdf.text(goldRatePerGram.toFixed(0), 125, currentY + 6, { align: 'center' })
    ...
    pdf.text(goldPortion.toFixed(0), 205, currentY + 6, { align: 'center' })
    pdf.text(makingChargeAmount.toFixed(0), 230, currentY + 6, { align: 'center' })
    pdf.text('0', 257.5, currentY + 6, { align: 'center' })
    pdf.text(item.amount.toFixed(0), 278.5, currentY + 6, { align: 'center' })
  })
  ...
  pdf.setFontSize(5)
  pdf.text('CGST @ 1.50% (gold only):', 194, taxY + 15)
  pdf.text(billData.totalCgst.toFixed(2), 225, taxY + 15)
  pdf.text('SGST @ 1.50% (gold only):', 194, taxY + 19)
  pdf.text(billData.totalSgst.toFixed(2), 225, taxY + 19)
  pdf.text('Making charges GST exempt & hallmarking disabled.', 194, taxY + 23)
